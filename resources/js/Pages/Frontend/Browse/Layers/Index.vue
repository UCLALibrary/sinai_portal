    ...
    <AisInstantSearch
      ...
      <div class="facet-sidebar">
        ...
        <div class="accordion-items">
          <AccordionCard title="Current State" class="accordion-item">
            <template v-slot:content>
              <AisDynamicWidgets :max-values-per-facet="maxFacetValuesToShow">
                <AisRefinementList
                  :limit="maxFacetValuesToShow"
                  attribute="state"
                  @change="onFilter('state', $event.target.value)"
          ...
          <AccordionCard title="Script" class="accordion-item">
            <template v-slot:content>
              <AisDynamicWidgets :max-values-per-facet="maxFacetValuesToShow">
                <AisRefinementList
                  :limit="maxFacetValuesToShow"
                  attribute="script"
                  @change="onFilter('script', $event.target.value)"
                  class="max-h-48 overflow-y-scroll my-4">
                  <template v-slot="{ items }">
                    <div v-if="!items.length">
                      No results found
          ...
          <AccordionCard title="Writing System" class="accordion-item">
            <template v-slot:content>
              <AisDynamicWidgets :max-values-per-facet="maxFacetValuesToShow">
                <AisRefinementList
                  :limit="maxFacetValuesToShow"
                  attribute="writing_system"
                  @change="onFilter('writing_system', $event.target.value)"
                  class="max-h-48 overflow-y-scroll my-4">
                  <template v-slot="{ items }">
                    <div v-if="!items.length">
                      No results found
